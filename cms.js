...
hbs.registerHelper('calcTotalPrice', function (cart) {
    let eachPrice = cart.map(
        (val) =>
            Number(val.quantity) *
            Number(val.product.sale_price || val.product.price),
    );
    ...
    async (req, res, next) => {
        ...
        if (req.session && req.session.hasOwnProperty('person')) {
            switch (true) {
                ...
                case myFuncs['moduleRole'][req.params.module] == 'admin' &&
                    req.session.person.role == 'auth':
                    return res.send(
                        'you are trying to access admin page while your role is auth only',
                    );
                    break;
                // Auth is accessing an auth role
                case (req.session.person.role == 'auth',
                ...
                case req.session.person.role == req.params.permit &&
                    req.session.person.brand
                        .split(',')
                        .some((val) => val.trim() == req.params.brand):
                    return next();
                    break;
        ...
        let collectionsSavedCheck = await Collections.findOne({
            name: 'collections',
        }).lean();
        ...
        let checkCollectionExists = await myFuncs.checkCollectionExists(
            `${req.params.brand}-users`,
        );

        if (checkCollectionExists) {
            if (
                req.params.requiredType != 'page' &&
                req.params.requiredType != 'pjax'
            )
                return res
                    .status(404)
                    .send(
                        'sign in is required or may be module does not exist',
                    );
            return res.redirect(`/${req.params.brand}/gen/page/landingPage/n/`);
        }

        checkCollectionExists = await myFuncs.checkCollectionExists(
            `myapp-users`,
        );
...
let openBrand = async (req, res) => {
    if (req.params.brand.indexOf('.') > 0) {
        return res
            .status(300)
            .send(
                'Sorry, this url contains a dot which is a wrong attempt. Haha keep trying..',
            );
    }
...
let myFuncs = {
    respond: async function (data = {}, req, res) {
        ...
        try {
            Object.assign(data, {
                ...
                query: req.query,
                iframe: await this.getThemeURL(req.params.brand),
            });
        } catch (e) {
            console.log(e);
        }
        ...
        let storeThisFileInSession = async function () {
            req.session.file = await new Promise((resolve, reject) => {
                fs.readFile(
                    `./views/${req.params.theme}/${req.params.module}.hbs`,
                    'utf8',
                    (err, data) => {
                        if (err) {
                            console.log(err);
        ...
        switch (true) {
            case req.query.hasOwnProperty('redirect'):
                let url = `/${req.params.brand}/${req.params.permit}/${req.params.requiredType}/${req.query.redirect}/${req.query.redirectInput}`;
            ...
            case req.params.hasOwnProperty('pageName'):
                return res
                    .status(200)
                    .render(`${req.params.theme}/${req.params.pageName}.hbs`, {
                        data,
                    });
                break;
            case req.headers['x-pjax'] == 'true':
                return res
                    .status(200)
                    .render(
                        `${req.params.theme}/pjax/${req.params.module}.hbs`,
                        { data },
                    );
                break;
            case req.query.hasOwnProperty('webEdit') &&
                req.query.webEdit == 'true' &&
                req.session.hasOwnProperty('person'):
                storeThisFileInSession();
                return res
                    .status(200)
                    .render(`${req.params.theme}/${req.params.module}.hbs`, {
                        data,
                        webEdit: true,
                    });
                break;
            case req.params.requiredType == 'email':
                return res
                    .status(200)
                    .render(`emails/${req.params.module}.hbs`, { data });
                break;
            default:
                return res
                    .status(200)
    ...
    shipEmail: async function (req, res) {
        ...
        await list.forEach(async (val) => {
            arr.push({
                list: val,
                emails: await model
                    .find({ list: val, isUnsubscribed: 'false' })
                    .lean(),
            });
        });
    ...
    sendNewsletter: async function (req, res) {
        ...
        if (
            !(
                req.session.newsletter &&
                req.session.newsletter._id == req.body.id
            )
        ) {
            let output;
    ...
    newDashboard: async function (req, res) {
        req.params.theme = 'root';
        req.params.module =
            req.headers['x-pjax'] == 'true' ? req.params.input : 'newDashboard';
        ...
        if (req.params.input.match(/-/g) == null) {
            switch (true) {
                ...
                case req.params.input == 'settings':
                    ...
                    model2 = await this.createModel(
                        `${req.params.brand}-users`,
                    );
                    output.themeDetails = await model
                        .findOne({ brand: req.params.brand })
                        .lean();
                    output.person = await model2
                        .findOne({ email: req.session.person.email })
                        .lean();
            ...
            let collectionsTable = await Collections.find({
                brand: req.params.brand,
            }).lean();
            ...
            if (collectionsTable.length == 0)
                return {
                    status: 200,
                    success:
                        'no Collection exists yet. Try starting the app with basic configurations.',
                    brand: req.params.brand,
                };
            ...
            let requestedCollection = collectionsTable.find(
                (val) => val.name == req.params.input,
            );

            let collectionHeadings = Object.keys(
                collectionsTable.find((val) => val.name == req.params.input)
                    .properties,
            );

            collectionHeadings.unshift('_id');

            collectionHeadings = collectionHeadings.filter(
                (val) => val != 'fixed' && val != 'noClone',
            );

            model = await this.createModel(req.params.input);

            let dataRows = await model
                .find()
                .sort({ ser: 1 })
            ...
            if (dataRows.length == 0) {
                ...
                for (i = 0; i < collectionHeadings.length; i++) {
                    switch (true) {
                        ...
                        default:
                            property =
                                requestedCollection.properties[
                                    collectionHeadings[i]
                                ];
                    }
            ...
            req.params.module =
                req.headers['x-pjax'] == 'true' ? 'rootTable' : 'newDashboard';

            let thisCollection = await Collections.findOne({
                name: req.params.input,
            }).lean();

            output = {
                schema: collectionsTable.find(
                    (val) => val.name == req.params.input,
                ).properties,
                th: newHeadings,
                dataRows: newRows,
                ...
                airtable: {
                    basePin:
                        thisCollection.airtable &&
                        thisCollection.airtable.baseId,
                    baseAPIKey:
                        thisCollection.airtable &&
                        thisCollection.airtable.baseAPIKey,
                    tableName:
                        thisCollection.airtable &&
                        thisCollection.airtable.tableName,
                },
            };
        }
    ...
    listenToWebhook: function (req, res) {
        if (req.body && req.body.event) {
            if (req.body.event.hasOwnProperty('attachments') == false)
                return {
                    status: 300,
    ...
    syncFromAirtableToLocal: async function ({ data, brand }) {
        ...
        let pipe_airtable_upload = async (record) => {
            ...
            let airtableURLs = (
                await Collections.findOne({ name: collectionName }).lean()
            ).airtable;
            ...
            let storeNow = await record.model.findOneAndUpdate(
                { [airtableURLs.key]: remoteData[airtableURLs.key] },
                remoteData,
                { new: true },
            );
        ...
        let output = await Promise.all(
            data.map((val) => pipe_airtable_upload(val)),
        );
    ...
    syncWithAirtable: async function ({ collection, data }) {
        let airtableURLs = (
            await Collections.findOne({ name: collection }).lean()
        ).airtable;
        let properties = (
            await Collections.findOne({ name: collection }).lean()
        ).properties;
        ...
        let formatData = function (matchingID, data) {
            let dateProperty = Object.keys(properties).find(
                (val) => properties[val]['type'] == 'Date',
            );

            if (dateProperty != undefined) {
                data[dateProperty] = myFuncs.getFormattedDate(
                    data[dateProperty],
                );
            }
        ...
        let saveNewKeyInLocalDB = async function (dataToStore) {
            ...
            let deleteOldKeys = await Collections.findOneAndUpdate(
                { name: collection },
                {
                    $pull: {
                        'airtable.connectingKeys': {
                            fields: {
                                [airtableURLs.key]: data[airtableURLs.key],
                            },
                        },
                    },
                },
            ...
            let newStore = await Collections.findOneAndUpdate(
                { name: collection },
                { $push: { 'airtable.connectingKeys': dataToStore } },
                { new: true },
            ).lean();
        ...
        let pushNewToAirtable = async function (matchingID, data) {
            ...
            let newUpload = await myFuncs.axiosRequest({
                method: 'POST',
                data: formatData(matchingID, data),
                URL: airtableURLs.post,
            });
        ...
        let lookForValueinAirtable = async function (matchingID, data) {
            let connectingKeys = (
                await myFuncs.axiosRequest({
                    method: 'GET',
                    URL: airtableURLs.get + '&fields=' + airtableURLs.key,
                })
            ).data.records;

            let connectingKey = connectingKeys.find(
                (val) => val.fields[airtableURLs.key] == data[airtableURLs.key],
            );

            if (connectingKey == undefined)
                return await pushNewToAirtable(undefined, data);
            ...
            let updateToAirtable = await myFuncs.airtableAPI({
                method: 'update',
                data: formatData(matchingID, data),
                baseId: airtableURLs.baseId,
                baseName: airtableURLs.baseName,
            });

            if (
                updateToAirtable &&
                updateToAirtable.response &&
                updateToAirtable.response.data.hasOwnProperty('error')
            ) {
        ...
        let matchingID = airtableURLs.connectingKeys.find(
            (val) =>
                val &&
                val.fields &&
                val.fields[airtableURLs.key] == data[airtableURLs.key],
        );
        ...
        let updateToAirtable = await this.axiosRequest({
            method: 'PUT',
            data: formatData(matchingID, data),
            URL: airtableURLs.put,
        });
    ...
    getThemeURL: async function (brand) {
        try {
            const model = await this.createModel('myapp-themes');
            const settings = await model.findOne({ brand: brand }).lean();
    ...
    getDirectoryList: async function (directoryName) {
        return new Promise((resolve, reject) => {
            fs.readdir(
                directoryPath + '/views/' + directoryName,
                function (err, files) {
                    if (err) {
    ...
    getFormInputs: async function (req, res) {
        ...
        let output = keys.map((val) => {
            if (
                properties[val]['html'] == 'imgURL' &&
                req.values &&
                req.values[val] != undefined
            ) {
    ...
    fetchResources: async function (req, res) {
        try {
            let resources = await this.createModel(
                `${req.params.brand}-resources`,
            );
    ...
    editDocument: async function (req, res) {
        ...
        if (req.values == undefined)
            return {
                status: 404,
                error: 'document does not exist so it can not be edited',
            };
        let inputs = await this.getFormInputs(req, res);
    ...
    updateSequence: async function (req, res) {
        ...
        if (result == undefined)
            return { status: 404, error: 'did not find matching document' };
    ...
    createModel: async function (modelName) {
        try {
            let modelExistsAlready = Object.keys(mongoose.models).some(
                (val) => val == modelName,
            );
            let schemaExistsAlready =
                mongoose.modelSchemas &&
                Object.keys(mongoose.modelSchemas).some(
                    (val) => val == modelName,
                );
            ...
            return (
                ...
                mongoose.model(
                    modelName,
                    new mongoose.Schema(schema.properties, {
            ...
            console.log(
                chalk.red.bold('Failed to create Model' + ':' + modelName),
            );
            console.log(e);
            return e;
        }
    ...
    airtableSync: async function (req, res) {
        req.params.theme = 'root';
        let output = await Collections.findOne({
            name: req.params.input,
        }).lean();
        return {
            modelName: req.params.input,
        ...
        try {
            ...
            switch (true) {
                ...
                case method == 'update':
                    ...
                    output = await base(baseName).update(data, {
                        typecast: true,
                    });
                    break;
    ...
    saveAirtableURLs: async function (req, res) {
        let keysAll = await this.airtableAPI({
            ...
            baseName: req.body.tableName,
            baseAPIKey: req.body.baseAPIKey,
            method: 'test',
        });
    ...
    importAndMerge: async function (req, res) {
        try {
            let myProperties = await Collections.findOne({
                ...
                Object.entries(val).forEach((love) => {
                    switch (true) {
                        case cp[love[0]] == undefined:
                            console.log(
                                love[0] + ' is not wanted - removing it',
                            );
                            delete val[love[0]];
                            break;
            ...
            let model = await this.createModel(req.params.input);
            await Promise.all(
                keysAll.map((val) => model.deleteOne({ ser: val.ser })),
            );
    ...
    updateManyByKey: async function (req, res) {
        let model = await this.createModel(`${req.params.input}`);
        let output = await Promise.all(
            req.body.data.map((val, index) =>
                model.findOneAndUpdate(
                    { [val.key]: val.data[val.key] },
                    val.data,
    ...
    fetchAirtableData: async function (req, res) {
        let airtableURLs = (
            await Collections.findOne({ name: req.params.input }).lean()
        ).airtable;

        let output = await this.axiosRequest({
            method: 'GET',
            URL: airtableURLs.get,
        });
    ...
    airtableVslocal: async function (req, res) {
        ...
        if (
            airtableURLs.put == undefined ||
            airtableURLs == undefined ||
            file == undefined
        ) {
            req.query.redirect = 'airtableSync';
            req.query.redirectInput = req.params.input;
            return {
                msg: 'The PUT URL is not listed in the form. Please update PUT URL and press the Analyze btn',
            };
        }
        ...
        let localToAirtable = output2.map((val) => {
            let matchInAirtable = file.find(
                (row) =>
                    row['fields'][airtableURLs.key] == val[airtableURLs.key],
            );
            return {
                local: val,
        ...
        let airtableToLocal = file
            .map((val) => {
                let matchInLocal = output2.find((row) => {
                    ...
                    return (
                        val['fields'][airtableURLs.key] == row[airtableURLs.key]
                    );
                });
        ...
        let getDifferences = function (focal) {
            if (focal.local == undefined || focal.airtable == undefined) {
                ...
                let diff = makeObjectAnArray(
                    focal.local == undefined
                        ? focal.airtable.fields
                        : focal.local,
                    focal.local == undefined ? 'airtable' : 'local',
                );

                return {
                    local:
                        focal.local == undefined
                            ? (focal.local = { _id: myFuncs.getMongoId() })
                            : focal.local,
            ...
            let output = Object.keys(focal.local)
                .map((val) => {
                    if (
                        val == 'created_at' ||
                        val == 'updatedAt' ||
                    ...
                    let diff = myFuncs
                        .patienceDiffPlus(
                            focal.local[val] == undefined
                                ? ' '
                                : focal.local[val],
                            focal.airtable.fields[val] == undefined
                                ? ' '
                                : focal.airtable.fields[val],
                        )
                        .lines.filter((val) => val.bIndex == -1);
        ...
        let rows = sidebyside
            .map((val) => getDifferences(val))
            .filter((val) => val != '')
            .map((val) => {
                val.keyValue =
                    val.local[airtableURLs.key] ||
                    val.airtable[airtableURLs.key];
    ...
    patienceDiff: function (aLines, bLines, diffPlusFlag) {
        ...
        function addSubMatch(aLo, aHi, bLo, bHi) {
            ...
            let uniqueCommonMap = uniqueCommon(
                aLines,
                aLo,
                aHi,
                bLines,
        ...
        function recurseLCS(aLo, aHi, bLo, bHi, uniqueCommonMap) {
            var x = longestCommonSubsequence(
                uniqueCommonMap ||
                    uniqueCommon(aLines, aLo, aHi, bLines, bLo, bHi),
            );
            if (x.length === 0) {
    ...
    dropCollection: async function (req, res) {
        try {
            let collectionName =
                req.params.input.indexOf('-') > 0
                    ? req.params.input.split('-')[1]
                    : req.params.input;
            let output = await Collections.remove({
                name: `${req.params.brand}-collectionName`,
            });
    ...
    editCollection: async function (req, res) {
        ...
        let collectionDetails = await Collections.findOne({
            name: req.params.input,
        }).lean();
        let output = Object.keys(collectionDetails.properties)
            .map((val) => {
                ...
                return {
                    ...
                    html:
                        collectionDetails.properties[val].html == undefined
                            ? 'input'
                            : collectionDetails.properties[val].html,
                    allowedValues:
                        collectionDetails.properties[val].allowedValues,
    ...
    showCollection: async function (req, res) {
        ...
        let inputCollection = await Collections.findOne({
            name: req.params.input,
        }).lean();
        ...
        if (
            ...
            req.query.redirect =
                (inputCollection.redirect &&
                    inputCollection.redirect.split('/')[0]) ||
                'showCollection';
            req.query.redirectInput =
                (inputCollection.redirect &&
                    inputCollection.redirect.split('/')[1]) ||
                'n';
            return {
                status: 200,
                success:
                    'Request is now redirected to redirected page mentioned inside the database',
                brand: req.params.brand,
            };
        }
    ...
    deleteDocument: async function (req, res) {
        let model = await this.createModel(
            `${req.params.brand}-${req.params.input}`,
        );
    ...
    createSignUp: async function (req, res) {
        ...
        let checkAllowed = await appModel
            .findOne({ brand: req.params.brand })
            .lean();
    ...
    checkSignIn: async function (req, res) {
        ...
        if (!output)
            return {
                status: 400,
                error: 'Email Password Mismatch. Please Sign Up.',
            };
    ...
    uploadMany: async function (req, res) {
        let model = await this.createModel(req.params.input);
        let mongoId = req.body.data.map((val) =>
            val._id.match(/^[0-9a-fA-F]{24}$/)
                ? val._id
        ...
        let output = await Promise.all(
            req.body.data.map((val, index) =>
                model.findOneAndUpdate({ _id: mongoId[index] }, val, {
                    new: true,
                    upsert: true,
                }),
            ),
    ...
    ecommerce: async function (req, res) {
        ...
        models.resources = await this.createModel(
            `${req.params.brand}-resources`,
        );
        let output = await Promise.all([
    ...
    blogStreak: async function (req, res) {
        ...
        for (let i = 0; i < difference; i++) {
            this_date = new Date(
                new Date(start_date).getTime() + 1000 * 60 * 60 * 24 * i,
            );
            array.push({
                date: this_date,
                blog: output.find((val) => {
                    var diff = Math.floor(
                        (Date.parse(val.date) - Date.parse(this_date)) /
                            86400000,
                    );
    ...
    deleteComment: async function (req, res) {
        ...
        if (!(req.session.person && req.session.person.email))
            return {
                status: 404,
    ...
    portfolio: async function (req, res) {
        ...
        let related_projects = await Promise.all(
            output.related_projects
                .split(',')
                .map((val) => model.findOne({ slug: val }).lean()),
        );
        return {
            project: output,
            related_projects,
    ...
    language_data: function (req, returned) {
        ...
        req.session.lang =
            req.session && req.session.hasOwnProperty('lang')
                ? req.session.lang
                : 'en';
    ...
    changePassword: async function (req, res) {
        let model = await this.createModel(`${req.params.brand}-users`);
        let output1 = await model.findOne({
            email: req.session.person.email,
            password: req.body.oldPass,
        });
        if (output1 == null)
            return { status: 404, error: 'Old password does not match!' };
        let output2 = await model.findOneAndUpdate(
            { _id: req.body._id },
            {
                $set: {
        ...
        if (output2 == null)
            return { status: 400, error: 'Something bad happened!' };
        return { success: true };
    },

    updateDocumentInThemes: async function (req, res) {
        if (req.query._id == '')
    ...
    getSummaryOfTasks: async function (req, res) {
        ...
        let subjects = await model
            .distinct('subject', { email: req.session.person.email })
            .lean();
    ...
    userSignedUp: async function (req, res) {
        ...
        if (!brand.signUpAllowed)
            return {
                status: '404',
                error: 'Sign up is not enabled for this brand',
            };
        let length = 10000000000000000000000000000000000;
        let code = (Math.floor(Math.random() * length) + length)
            .toString()
            .substring(1);
        let model = await this.createModel(`${req.params.brand}-users`);
        let checkUserExists = await model
    ...
    d_pmodules: {
        ...
        threePages: async function (req, res) {
            ...
            return {
                ...
                helpAndSupport: await model
                    .findOne({ slug: 'help-and-support' })
                    .lean(),
                volunteering: await model
                    .findOne({ slug: 'volunteering' })
                    .lean(),
            };
        },

        gallery: async function (req, res) {
            let model = await myFuncs.createModel(
                `${req.params.brand}-gallery`,
            );
            let output = await model.find().lean();
            output = output.map((val) => {
                val.number = val.url
                    .split('/image/upload/')[1]
                    .split('/dedicatedparents/')[0];
    ...
    d_ppage: async function (req, res) {
        let model = await this.createModel(`${req.params.brand}-blogs`);
        let modelComments = await this.createModel(
            `${req.params.brand}-comments`,
        );
        let output = await model.findOne({ slug: req.params.input }).lean();
        output.number = output.bannerImg
            .split('/image/upload/')[1]
            .split('/dedicatedparents/')[0];
        ...
        return {
            ...
            comments: await this.getComments(
                req.params.brand,
                output,
                req.query.uniqueCode,
            ),
            countComments: await modelComments
                .count({ slug: output.slug })
                .lean(),
            gallery: await this.d_pmodules.gallery(req, res),
            footerBlogs: await this.d_pmodules.footerBlogs(req, res),
        };
    ...
    getComments: async function (brand, blog, uniqueCode) {
        ...
        let output = await model
            .find({ slug: blog.slug, replyTo: 'none' })
            .lean();
        let result;
    ...
    landingPage: async function (req, res) {
        if (this[req.params.theme] == undefined) {
        ...
        const output = await this[req.params.theme](req, res);
        if (output) return output;
    },

    mongoQueries: async function (req, res) {
        let model = await (
            await this.createModel(`${req.params.brand}-${req.params.input}`)
        )
            .find({})
            .lean();
        let models = await Promise.all(
            model.map((val) => this.createModel(val.collectionName)),
        );
        let output = await Promise.all(
        ...
        output = output.reduce((total, val, index) => {
            switch (true) {
                case model[index]['specialMethod'] &&
                    model[index]['specialMethod'].length != 0:
                    let specialMethodName =
                        model[index]['specialMethod'].split(' ')[0];
                    let dataKeyName =
                        model[index]['specialMethod'].split(' ')[1];
                    val = val.map((v) => {
                        v[dataKeyName] = this[specialMethodName](
                            v[dataKeyName],
                        );
                        return v;
                    });
    ...
    searchItems: async function (req, res) {
        ...
        if (req.body.itemName && req.body.itemName.length > 0)
            query.name = new RegExp(req.body.itemName, 'i');
        if (
            req.body.selectedCategories &&
            req.body.selectedCategories.length > 0
        )
            query.category = { $in: req.body.selectedCategories };
        if (req.body.selectedSchools && req.body.selectedSchools.length > 0)
            query.school = { $in: req.body.selectedSchools };
        ...
        let result = await model.aggregate([
            {
                $match: {
                    $or: [
                        ...
                        {
                            email:
                                req.session.person && req.session.person.email,
                        },
                    ],
                    cartStatus: req.params.hasOwnProperty('cartStatus')
                        ? req.params.cartStatus
                        : 'open',
                },
            },
            {
    ...
    cartPage: async function (req, res) {
        ...
        let output = {
            ...
            sessionId: req.sessionID,
            email:
                req.session.person && req.session.person.email.length > 0
    ...
    updateCart: async function (req, res) {
        ...
        cart = cart.map((val) => {
            val._id =
                val._id == ''
                    ? new mongoose.mongo.ObjectID()
                    : mongoose.mongo.ObjectID(val._id);
            return val;
        });
        let output = await Promise.all(
            cart.map((val, index) =>
                model.findOneAndUpdate({ _id: val._id }, val, {
                    new: true,
                    upsert: true,
                }),
            ),
    ...
    removeItemFromCart: async function (req, res) {
        ...
        return out.deletedCount == 0
            ? { status: 300, error: 'item did not remove' }
            : { success: 'item removed', output: out };
    },
    ...
    reviewCartPage: async function (req, res) {
        ...
        let totalCost = result.reduce((total, val, index) => {
            total =
                Number(total) +
                Number(val.quantity) * Number(val.items[0].cost);
            return total;
        }, 0);
        let output = {
            resources: resources,
            ...
            totalCost: totalCost,
            order: myOrder.order,
            orderNo: this.padWithZeroes(
                Number(myOrder.orderNo.replace(/^0+/, '')) + 1,
                6,
            ),
            brand: req.params.brand,
            sessionId: req.sessionID,
    ...
    createOrder: async function (req, res) {
        ...
        let output = model.findOneAndUpdate({ _id: order._id }, order, {
            upsert: true,
            new: true,
        });
        ...
        let slackNotify = this.notifySlack(
            resources[0].slackToken,
            msg,
            resources[0].slackChannelId,
        );
    ...
    findOrderInSession: async function (req, res) {
        ...
        return {
            order: output,
            orderNo:
                highestOrder.length > 0 ? highestOrder[0].orderNo : '000000',
        };
    },
    ...
    saveItemInCart: async function (req, res) {
        ...
        req.body.email = req.session.hasOwnProperty('person')
            ? req.session.person.email
            : 'false';
        let output = await model
            .findOneAndUpdate(
    ...
    showOrders: async function (req, res) {
        ...
        let finalOrdersOutput = orders.map((val, index) => {
            return {
                ...
                totalCost: ordersWithItems[index].reduce(
                    (total, val, index) => {
                        total =
                            Number(total) +
                            Number(val.quantity) * Number(val.items[0].cost);
    ...
    updateOrder: async function (req, res) {
        ...
        let output = await model.findOneAndUpdate(
            { _id: order._id },
            { $set: { status: order.status, payment: order.payment } },
            { new: true },
        );
        let result = {
            order: output,
    ...
    orderDelivered: async function (req, res) {
        ...
        switch (true) {
            case req.body.quantityTest == -1:
                items = items.map((val) => {
                    val.newQty =
                        Number(val.quantity) - Number(val.quantityDiff);
                    return val;
                });
                break;
            case req.body.quantityTest == 1:
        ...
        let output = await Promise.all(
            items.map((val) =>
                model
                    .findOneAndUpdate(
                        { _id: mongoose.Types.ObjectId(val.itemId) },
                        { $set: { quantity: val.newQty } },
                        { new: true },
                    )
                    .lean(),
            ),
    ...
    showProperty: async function (req, res) {
        ...
        property.genSelected =
            req.session.Cards &&
            req.session.Cards.myArray &&
            req.session.Cards.myArray.some(
                (card) => card._id == property._id.toString(),
            )
                ? 'select'
                : '';
        let suggestedProperties = await model
            .find({
                $and: [
                    { city: property.city },
                    {
            ...
            .limit(4)
            .lean();
        suggestedProperties = this.matchSelectedProperties(
            req,
            res,
            suggestedProperties,
        );
        let output = {
            ...
            forms: await this.getForms(
                { msgBoxClient: true, contactForm: true },
                req,
                res,
            ),
    ...
    dashboard: async function (req, res) {
        ...
        let counts = {
            netRevenue: output.reduce(
                (total, val) => (total = total + val.netRevenue),
                0,
            ),
            outOfStock: await models.items
                .find({ quantity: 0 })
                .countDocuments(),
            unhandledOrders: await models.orders
                .find({ status: 'Order Placed in Store' })
                .countDocuments(),
    ...
    calcProfits: async function (req, from, to) {
        ...
        let output = await model.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    purchaseCost: {
                        $sum: { $multiply: ['$purchaseCost', '$itemQuantity'] },
                    },
                    sellCost: {
                        $sum: { $multiply: ['$sellCost', '$itemQuantity'] },
                    },
                },
            },
            {
        ...
        let grossResult = {
            ...
            purchaseCost: output.reduce(
                (total, val) => (total = total + val.purchaseCost),
                0,
            ),
            sellCost: output.reduce(
                (total, val) => (total = total + val.sellCost),
                0,
            ),
            netRevenue: output.reduce(
                (total, val) => (total = total + val.netRevenue),
    ...
    profits: async function (req, res) {
        var date = new Date();
        let from =
            (req.query.from && new Date(req.query.from).setHours(00, 00, 00)) ||
        ...
        let to =
            (req.query.to && new Date(req.query.to).setHours(23, 59, 59)) ||
        ...
        let grossResult = {
            totalOrders: output.reduce((total, val) => (total = total + 1), 0),
            purchaseCost: output.reduce(
                (total, val) => (total = total + val.orderPurchasePrice),
                0,
            ),
            sellCost: output.reduce(
                (total, val) => (total = total + val.orderSellPrice),
                0,
            ),
            netRevenue: output.reduce(
                (total, val) => (total = total + val.netRevenue),
        ...
        let orders = await model.aggregate([
            ...
            },
            {
                $project: {
                    ...
                    netRevenue: {
                        $subtract: ['$orderSellPrice', '$orderPurchasePrice'],
                    },
                },
            },
            {
    ...
    allOrders: async function (req, res) {
        ...
        query =
            Object.values(req.query.filter).length > 0 ? req.query.filter : {};
        let output = await this.fetchOrders(req, query);
    ...
    searchDashboard: async function (req, res) {
        ...
        let output = {
            ...
            users: await models.users
                ...
                .limit(4),
            resources: await models.resources
                .find(
                    {
                        $or: [
                            {
    ...
    createAppBackup: async function (req, res) {
        ...
        let models = await Promise.all(
            names.map((val) => this.createModel(val.name)),
        );

        let outputs = await Promise.all(
            models.map((val, index) => val.find({})),
        );
    ...
    loadBackUp: async function (req, res) {
        ...
        let collectionFile = file.filter(
            (val) => val.name === 'collections',
        )[0];

        try {
            let collectionDrop = await mongoose.connection.db.dropCollection(
                'collections',
            );
        } catch (e) {
            console.log(e);
        ...
        let collectionSave = await Promise.all(
            collectionFile.data.map((val) =>
                Collections.findOneAndUpdate({ _id: val._id }, val, {
                    upsert: true,
                }),
            ),
        );
        ...
        let models = await Promise.all(
            file.map((val) => this.createModel(val.name)),
        );
        ...
        let emptyAllCollections = await Promise.all(
            funcs.map((val) => val.model.deleteMany({})),
        );
        let outputs = await Promise.all(
            funcs.map((val) =>
                val.model.findOneAndUpdate({ _id: val.data._id }, val.data, {
                    upsert: true,
                }),
            ),
        );
            ...
            switch (true) {
                case i == 1:
                    box.push({
                        ...
                        comments:
                            'I am feeling kind of happy today. Let`s see till what point can I get moving',
                    });
                    break;
                case i == 2:
                    box.push({
    ...
    subscribeCustomer: async function (req, res) {
        ...
        url =
            process.env.url +
            `/${req.params.brand}/gen/page/verifyEmail/n?email=${req.body.email}&uniqueCode=${output._id}`;

        let mailResponse;
        let mailModel = await this.createModel(
            `${req.params.brand}-newsletters`,
        );
        ...
        logEntry = await logModel.create({
            status: 200,
            text: `Email ${(mail && mail.slug) || 'verify-email'} sent to ${
                output.email
            }`,
    ...
    lifeTimer: async function (req, res) {
        ...
        let pendingLetters = await model
            .find({
                publishTime: {
                    $gte: new Date(),
                },
        ...
        if (pendingLetters.length == 0)
            return {
                success: 'Currently there are no newsletters pending Publish',
            };
    ...
    setTimerToPublish: function (letter) {
        ...
        setTimeout(
            async () => {
                ...
                let mailSent = await this.sendBulkEmailWithTemplate(
                    'life',
                    letter.slug,
                    output,
                );
        ...
        return `Publishing <b>${letter.slug}</b> in - ${Math.floor(
            millisTillPublish / 1000 / 60,
        )} minutes`;
    },

    sendBulkEmailWithTemplate: async function (
        brand,
        templateSlug,
        subscribers,
    ) {
    ...
    sendEmailWithTemplate: async function (brand, templateSlug, subscriber) {
        ...
        if (output == null)
            return console.log(
                chalk.bold.red(
                    'COULD NOT SEND MAIL BECAUSE SLUG WAS NOT FOUND',
                ),
            );
        ...
        let sentMails = await this.sendMail({
            ...
            context: {
                ...
                unsubscribeUrl:
                    process.env.url +
                    `/life/gen/page/unsubscribeMe/n?email=${subscriber.email}&Id=${subscriber._id}`,
            },
    ...
    unsubscribeMe: async function (req, res) {
        let model = await this.createModel(`${req.params.brand}-subscribers`);
        let output = await model
            .deleteOne({ _id: req.query.Id, email: req.query.email })
            .lean();
    ...
    fetchAirtable: async function (value) {
        ...
        switch (true) {
            case value == 'todos':
                string =
                    'https://v1.nocodeapi.com/punch__lines/airtable/WuNbPFGxKERSqrOC?tableName=features';
                break;
            default:
                return {
                    status: 404,
    ...
    getJSONFile: async function (req, res) {
        ...
        if (req.params.input == 'n') {
            ...
            file = JSON.parse(
                await readJSONFile(
                    `./static/${req.params.theme}/${req.params.input}.json`,
                ),
            );
        }
    ...
    readFromJSON: async function (req, res) {
        try {
            ...
            console.log(
                'create fake sessions for this theme like order and all to make sure all pages open just fine',
            );
            ...
            return {
                msg: 'hello world',
                brand: req.params.brand,
                manualInput: req.query.hasOwnProperty('manualInput')
                    ? req.query.manualInput
                    : 'n',
    ...
    editWeb: async function (req, res) {
        ...
        if (req.params.input == 'n') {
            ...
            file = await readHBSFile(
                `./views/${theme}/${req.params.input}.hbs`,
            );
        }
    ...
    saveWeb: async function (req, res) {
        ...
        let file = await writeFile(
            `./views/${req.params.theme}/${req.params.input}.hbs`,
            req.body.html,
        );
    ...
    createdProj: async function (req, res) {
        let missingValues = Object.values(req.body).some(
            (val) => val.length == 0,
        );
        ...
        if (missingValues == true) {
            return redirect(
                'Some values are missing. Please fill out complete form.',
            );
        }
        ...
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir);
            let files = req.body.files.includes(',')
                ? req.body.files.split(',')
                : ['landingPage', req.body.files];
        ...
        let checkEarlierExists = async function ({ brandName }) {
            let count = {
                inCollectionUsers: await Collections.find({
                    name: brandName + '-users',
                }).count(),
                inCollectionNotifications: await Collections.find({
                    name: brandName + '-notifications',
                }).count(),
        ...
        let checkPrvs = await checkEarlierExists({
    ...
    showProj: async function (req, res) {
        ...
        let getData = async function ({ brandName }) {
            let data = {
                ...
                inCollectionNotifications: await Collections.findOne({
                    name: brandName + '-notifications',
                }).count(),
    ...
    property: async function (req, res) {
        ...
        if (req.params.permit == 'gen') {
            output.forms = await this.getForms(
                { msgBoxClient: true, contactForm: true },
                req,
                res,
            );
    ...
    showAll: async function (req, res) {
        ...
        output.forms = await this.getForms(
            { msgBoxAdmin: true, addProperty: true, editBusiness: true },
            req,
            res,
        );
    ...
    offlineRequired: async function (req, res) {
        if (req.params.permit == 'gen') {
            return {
                ...
                forms: await this.getForms(
                    { contactForm: true, msgBoxClient: true },
                    req,
                    res,
                ),
                salutation: '',
    ...
    getAllCards: function (req, res) {
        let allCards = {
            authCards:
                req.session.authCards &&
                req.session.authCards.myArray &&
                req.session.authCards.myArray.length > 0
                    ? req.session.authCards.myArray
                    : [],
            authCardsCount:
                req.session.authCards &&
                req.session.authCards.myArray &&
                req.session.authCards.myArray.length > 0
                    ? req.session.authCards.myArray.length
                    : 0,
            Cards:
                req.session.Cards &&
                req.session.Cards.myArray &&
                req.session.Cards.myArray.length > 0
                    ? req.session.Cards.myArray
                    : [],
            CardsCount:
                req.session.Cards &&
                req.session.Cards.myArray &&
                req.session.Cards.myArray.length > 0
                    ? req.session.Cards.myArray.length
                    : 0,
        };
    ...
    getFiltersStatus: function (filters, query) {
        filters.cities = filters.cities.map((val) => {
            query.cities && query.cities.match(val.city)
                ? (val.status = 'active')
                : '';
            ...
            val.towns = val.towns.map((town) => {
                if (val.status == 'active') {
                    return {
                        name: town,
                        status:
                            query.towns && query.towns.split(',').includes(town)
                                ? 'active'
                                : '',
                    };
                } else {
        ...
        filters.status = filters.status.map((val) => {
            if (query.status && query.status.length > 0) {
                return {
                    name: val,
                    status:
                        query.status && query.status.split(',').includes(val)
                            ? (val.status = 'active')
                            : '',
                };
            } else {
    ...
    getProperties: async function (req, res) {
        ...
        let output = await model.aggregate([
            ...
            {
                $sort: {
                    priceInNo: req.query.hasOwnProperty('sort')
                        ? Number(req.query.sort)
                        : -1,
                },
            },
        ]);
    ...
    matchSelectedProperties: function (req, res, properties) {
        properties = properties.map((val) => {
            val.authSelected =
                req.session.authCards &&
                req.session.authCards.myArray &&
                req.session.authCards.myArray.some(
                    (card) => card._id == val._id.toString(),
                )
                    ? 'select'
                    : '';
            val.genSelected =
    ...
    buildMongoQuery: function (req, res) {
        ...
        if (req.query.hasOwnProperty('cities') && req.query.cities.length > 1) {
            ...
            if (
                req.query.hasOwnProperty('towns') &&
                req.query.towns.split(',').length > 0
            ) {
    ...
    drawForm: async function (req, res) {
        let form = (
            await this.getForms({ [req.params.input]: true }, req, res)
        )[req.params.input];
                ...
                elems: [
                    {
                        ...
                        rows: 10,
                        value:
                            (req.session.msgBoxClient &&
                                req.session.msgBoxClient.msg) ||
                            'Sir I am interested in these properties. Kindly when free get in touch..',
                        default:
                            'Sir I am interested in these properties. Kindly when free get in touch..',
                        onkeyup:
                            "saveInSession(this, 'msgBoxClient'); changeConnected(this, 'contactForm')",
                        name: 'msg',
                    },
                    {
                        elem: 'button',
                ...
                elems: [
                    {
                        ...
                        rows: 10,
                        value:
                            (req.session.msgBoxAdmin &&
                                req.session.msgBoxAdmin.msg) ||
                            'Sir fresh properties for today; contact to inquire more, please.',
                        default:
                            'Sir fresh properties for today; contact to inquire more, please.',
                ...
                elems: [
                    {
                        ...
                        value:
                            ...
                            'Sir fresh properties for today; contact to inquire more, please.',
                        onkeyup:
                            "saveInSession(this, 'msgBoxClient'); changeConnected(this, 'msgBoxClient')",
                    },
                    {
                        elem: 'input',
                        label: 'YOUR NAME',
                    ...
                    },
                    {
                        ...
                        value:
                            (req.session.contactForm &&
                                req.session.contactForm.contact) ||
                            '',
                        name: 'contact',
                        type: 'text',
                    },
            ...
            object.editBusiness = {
                ...
                elems: [
                    ...
                    },
                    {
                        ...
                        value:
                            output == undefined ? 'xyz@asdf.com' : output.email,
                    },
                    {
                        elem: 'input',
    ...
    saveSlide: async function (req, res) {
        ...
        let output = await model
            .findOneAndUpdate(
                {
                    _id: req.params.input.match(/temp/g)
                        ? this.getMongoId()
                        : req.params.input,
                },
                {
    ...
    changeSlideSequence: async function (req, res) {
        ...
        let output = await Promise.all(
            req.body.slides.map((val) =>
                model.findOneAndUpdate(
                    { _id: val.id },
                    { $set: { sequence: val.sequence } },
                    { new: true },
                ),
            ),
        );
    ...
    matchDataWithHeadings: function (dataArray, headings) {
        dataArray = dataArray.map((val) => {
            ...
            headings.forEach((hdg) => {
                if (
                    /noClone|fixed/gi.test(hdg) == false &&
                    val[hdg] != undefined
                ) {
                    total.push({
                        [hdg]: val[hdg],
    ...
    fetchCollectionData: async function (req, res) {
        ...
        let output = await model.find().lean();
        let result = this.matchDataWithHeadings(
            output,
    ...
    deleteImgInCloudinary: async function (req, res) {
        let output = await cloudinary.uploader.destroy(
            req.params.brand + '/' + req.body.imgId,
    ...
    kalles: async function (req, res) {
        ...
        return {
            ...
            sale: await model
                .find({ sale: { $exists: true } })
                .limit(8)
                .lean(),
            categories: await model.distinct('category').lean(),
            randomProducts: await model.aggregate([{ $sample: { size: 5 } }]),
            cart: req.session.cart != undefined ? req.session.cart : [],
        };
    },
    ...
    kallesQuickView: async function (req, res) {
        try {
            ...
            let product = await model
                .findOne({ _id: mongoose.mongo.ObjectID(req.params.input) })
                .lean();
            let sizes = await model.aggregate([
                {
                    $match: {
    ...
    kallesChangeCartQty: async function (req, res) {
        let cartItem =
            req.session &&
            req.session.cart &&
            req.session.cart.find((val) => val.slug == req.body.slug);
    ...
    createDocWithArr: async function (collection, model, data) {
        let props = (await Collections.findOne({ name: collection }).lean())
            .properties;
        let arrays = [],
            items = [];
        ...
        Object.entries(props).forEach((owl) => {
            ...
            if (value.type == 'Array' && data[key] && data[key].length > 0) {
                ...
                data[key].forEach((val) =>
                    arrays.push({ ser: data.ser, key: key, val: val }),
                );
                delete data[key];
            }
    ...
    updateDocWithArr: async function (collection, model, data) {
        ...
        Object.entries(props).forEach((owl) => {
            ...
            if (value.type == 'Array' && data[key] && data[key].length > 0) {
                console.log('this is array - ', key);
                data[key].forEach((val) =>
                    arrays.push({ _id: data._id, key: key, val: val }),
                );
                delete data[key];
            }
        ...
        let order = await model.findOneAndUpdate(
            { _id: data._id },
            { $set: data },
            { new: true },
        );
    ...
    kallesUpdateOrder: async function (req, res) {
        let model = await this.createModel(`${req.params.brand}-orders`);
        let store = await this.updateDocWithArr(
            `${req.params.brand}-orders`,
            model,
            req.body,
        );
    ...
    kallesPlaceOrder: async function (req, res) {
        ...
        let store = await this.createDocWithArr(
            `${req.params.brand}-orders`,
            model,
            req.body,
        );
    ...
    sendOrderAlerts: async function (req, res, order) {
        let getDateTime = function (objectId) {
            let date =
                objectId != undefined
                    ? new Date(
                          parseInt(objectId.toString().substring(0, 8), 16) *
                              1000,
                      )
                    : new Date();
            let dtg = date.toString().split(' ');
        ...
        try {
            ...
            let items = meta.reduce((total, val, key) => {
                return (total += `${val.quantity} x ${val.product.name} (${
                    val.size.label
                }) with item cost of  pkr ${
...
View receipt at : ${process.env.url}/${
                req.params.brand
            }/gen/email/receipt/n?ser=${order.ser}&email=${order.email}

Receipt sent by Server.`;

            let model = await this.createModel('myapp-themes');
            let brandDetails = await model
                .findOne({ brand: req.params.brand })
                .lean();
            let checkSlack = await this.axiosRequest({
                method: 'POST',
                data: { text: msg },
                URL: brandDetails.brandSlackURL,
            });
    ...
    testPlaceOrder: async function (req, res) {
        let model = await this.createModel(`${req.params.brand}-orders`);
        let lastOrder = (
            await model.find().limit(1).sort({ $natural: -1 }).lean()
        )[0];
    ...
    receipt: async function (req, res) {
        let model = await this.createModel(`${req.params.brand}-orders`);
        let output = await model
            .findOne({ ser: req.query.ser, email: req.query.email })
            .lean();
    ...
    saveSlackAPI: async function (req, res) {
        let checkSlack = await this.axiosRequest({
            method: 'POST',
            data: { text: 'Hello boss testing the app' },
            URL: req.body.url,
        });
        if (checkSlack.data == 'ok') return { success: true };
            ...
            return {
                status: 404,
                error: 'Could not send on this webhook. Validate the webhook please. ',
            };
    },
...
<p> — ${req.query.brandName} </p>
    ...
    sendMsgToEmail: async function (req, res) {
        if (!(req.body.msgText && req.body.toEmail && req.body.msgSubject))
            return {
                status: 404,
    ...
    kallesShop: async function (req, res) {
        let findMatchInFilter = function (array, filter, cat) {
            return array.map((val) => {
                return {
                    value: val,
                    active:
                        filter[cat] && filter[cat]['$in']
                            ? filter[cat] &&
                              filter[cat]['$in'].some((match) => match == val)
                            : filter[cat] == val,
                };
            });
        };
        ...
        req.query = processQuery(req.query, { price: { dataType: 'string' } });
        if (req.query && req.query.filter && req.query.filter._pjax)
            delete req.query.filter._pjax;
        ...
        let count = await model.countDocuments(req.query.filter).lean();
        let filters = {
            schools: findMatchInFilter(
                await model.find().distinct('school').lean(),
                req.query.filter,
                'school',
            ),
            category: findMatchInFilter(
                await model.find().distinct('category').lean(),
                req.query.filter,
                'category',
            ),
            type: findMatchInFilter(
                await model.find().distinct('type').lean(),
                req.query.filter,
                'type',
            ),
            gender: findMatchInFilter(
                await model.find().distinct('gender').lean(),
                req.query.filter,
                'gender',
            ),
        };
        ...
        return {
            ...
            clearFilters:
                ...
                Object.getPrototypeOf(req.query.filter) === Object.prototype,
            pages: this.getPagination(count, req.query.skip, 12),
        };
    },
    ...
    kallesProduct: async function (req, res) {
        ...
        let school_products = await model
            .find({ school: product.school })
            .lean();
        ...
        let first_aval_size = product_details.sizes.find(
            (val) => val.stock > 0,
        );
